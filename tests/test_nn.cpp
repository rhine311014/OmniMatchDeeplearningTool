...
TEST(NNTest, LinearWithBias) {
    ...
    df::Linear layer(2, 2, true);
    // 20260319 ZJH 创建 [1, 2] 零输入
    auto input = df::Tensor::zeros({1, 2});
...
TEST(NNTest, SequentialForward) {
    ...
    pModel->add(std::make_shared<df::Linear>(4, 3));  // 20260319 ZJH 第一层：4->3
    pModel->add(std::make_shared<df::ReLU>());         // 20260319 ZJH ReLU 激活
    pModel->add(std::make_shared<df::Linear>(3, 2));  // 20260319 ZJH 第二层：3->2

    // 20260319 ZJH 创建 [5, 4] 输入（batch=5, features=4）
    auto input = df::Tensor::ones({5, 4});
...
TEST(NNTest, SGDStep) {
    ...
    auto param = df::Tensor::ones({2, 3});
    df::tensorSetRequiresGrad(param, true);  // 20260319 ZJH 设置需要梯度

    // 20260319 ZJH 简单前向传播：loss = sum(param * 2)
    auto doubled = df::tensorMulScalar(param, 2.0f);  // 20260319 ZJH param * 2
    auto loss = df::tensorSum(doubled);  // 20260319 ZJH sum(param * 2) = 2 * sum(param)

    // 20260319 ZJH 反向传播
    df::tensorBackward(loss);

    // 20260319 ZJH 验证梯度为 2.0（d(sum(2*param))/dparam = 2）
    auto grad = df::tensorGetGrad(param);
    ...
    std::vector<df::Tensor*> vecParams = {&param};
    df::SGD optimizer(vecParams, 0.1f);
...
TEST(NNTest, AdamStep) {
    ...
    auto param = df::Tensor::full({2, 2}, 2.0f);
    df::tensorSetRequiresGrad(param, true);  // 20260319 ZJH 设置需要梯度

    // 20260319 ZJH 简单前向 + 反向
    auto scaled = df::tensorMulScalar(param, 3.0f);  // 20260319 ZJH param * 3
    auto loss = df::tensorSum(scaled);  // 20260319 ZJH sum(param * 3)
    df::tensorBackward(loss);  // 20260319 ZJH 反向传播，grad = 3.0
    ...
    std::vector<df::Tensor*> vecParams = {&param};
    df::Adam optimizer(vecParams, 0.01f);  // 20260319 ZJH lr=0.01
    optimizer.step();  // 20260319 ZJH 执行一步更新
...
TEST(NNTest, LinearBackward) {
    ...
    auto input = df::Tensor::ones({4, 3});
    // 20260319 ZJH 前向传播
    auto output = layer.forward(input);
    // 20260319 ZJH 计算损失：sum(output)
    auto loss = df::tensorSum(output);
    ...
    auto gradWeight = df::tensorGetGrad(*vecParams[0]);
    ...
    if (vecParams.size() >= 2) {
        auto gradBias = df::tensorGetGrad(*vecParams[1]);
...
TEST(NNTest, CrossEntropyForward) {
    df::CrossEntropyLoss criterion;  // 20260319 ZJH 创建交叉熵损失

    // 20260319 ZJH 创建 logits [2, 3] 和 one-hot targets [2, 3]
    auto logits = df::Tensor::zeros({2, 3});
    ...
    auto targets = df::Tensor::zeros({2, 3});
...
TEST(NNTest, ModuleParameters) {
    ...
    auto pModel = std::make_shared<df::Sequential>();
    pModel->add(std::make_shared<df::Linear>(4, 3));  // 20260319 ZJH weight[4,3] + bias[1,3] = 2 个参数
    pModel->add(std::make_shared<df::ReLU>());         // 20260319 ZJH 无参数
    pModel->add(std::make_shared<df::Linear>(3, 2));  // 20260319 ZJH weight[3,2] + bias[1,2] = 2 个参数
...
TEST(NNTest, ZeroGrad) {
    ...
    // 20260319 ZJH 验证梯度存在
    auto vecParams = layer.parameters();
    auto gradBefore = df::tensorGetGrad(*vecParams[0]);
    ...
    for (auto* pParam : vecParams) {
        auto grad = df::tensorGetGrad(*pParam);
...
TEST(NNTest, TrainEvalMode) {
    // 20260319 ZJH 构建模型
    auto pModel = std::make_shared<df::Sequential>();
    auto pLinear1 = std::make_shared<df::Linear>(4, 3);
    auto pRelu = std::make_shared<df::ReLU>();
    auto pLinear2 = std::make_shared<df::Linear>(3, 2);
    pModel->add(pLinear1);
    pModel->add(pRelu);
    pModel->add(pLinear2);
