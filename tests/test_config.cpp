...
TEST(ConfigTest, DefaultConstruct) {
    om::Config config;  // 默认构造，m_jsonData 应为空 JSON object
...
TEST(ConfigTest, SaveAndLoad) {
    ...
    {
        auto result = om::Config::load(strPath);
...
TEST(ConfigTest, LoadNonExistent) {
    auto result = om::Config::load("nonexistent_file.json");
    EXPECT_FALSE(result.has_value());  // 加载应失败
    // 20260319 ZJH 验证错误码为 FileNotFound，确保错误类型分类正确
    EXPECT_EQ(result.error().code, om::ErrorCode::FileNotFound);
}
