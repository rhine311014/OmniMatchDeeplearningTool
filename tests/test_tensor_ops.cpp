...
TEST(TensorOpsTest, Add) {
    ...
    auto c = om::tensorAdd(a, b);  // 20260319 ZJH 执行逐元素加法
...
TEST(TensorOpsTest, Sub) {
    auto a = om::Tensor::fromData(std::vector<float>{5, 6, 7, 8}.data(), {4});
    auto b = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4}.data(), {4});
    auto c = om::tensorSub(a, b);  // 20260319 ZJH 执行逐元素减法
...
TEST(TensorOpsTest, Mul) {
    auto a = om::Tensor::fromData(std::vector<float>{2, 3, 4, 5}.data(), {4});
    auto b = om::Tensor::fromData(std::vector<float>{10, 20, 30, 40}.data(), {4});
    auto c = om::tensorMul(a, b);  // 20260319 ZJH 执行 Hadamard 积
...
TEST(TensorOpsTest, Div) {
    auto a = om::Tensor::fromData(std::vector<float>{10, 20, 30, 40}.data(), {4});
    auto b = om::Tensor::fromData(std::vector<float>{2, 4, 5, 8}.data(), {4});
    auto c = om::tensorDiv(a, b);  // 20260319 ZJH 执行逐元素除法
...
TEST(TensorOpsTest, Matmul2D) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4}.data(), {2, 2});
    auto b = om::Tensor::fromData(std::vector<float>{5, 6, 7, 8}.data(), {2, 2});
    auto c = om::tensorMatmul(a, b);  // 20260319 ZJH 执行 2x2 矩阵乘法
...
TEST(TensorOpsTest, MatmulNonSquare) {
    ...
    auto c = om::tensorMatmul(a, b);  // 20260319 ZJH 执行 [2,3]@[3,2] 矩阵乘法
...
TEST(TensorOpsTest, Reshape) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4, 5, 6}.data(), {2, 3});
    auto b = om::tensorReshape(a, {3, 2});  // 20260319 ZJH 2x3 reshape 为 3x2
...
TEST(TensorOpsTest, ReshapeFlatten) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4}.data(), {2, 2});
    auto b = om::tensorReshape(a, {4});  // 20260319 ZJH 2x2 flatten 为 1D 向量
...
TEST(TensorOpsTest, Transpose) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4, 5, 6}.data(), {2, 3});
    auto b = om::tensorTranspose(a, 0, 1);  // 20260319 ZJH 交换维度 0 和 1
...
TEST(TensorOpsTest, TransposeThenContiguous) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4, 5, 6}.data(), {2, 3});
    auto b = om::tensorTranspose(a, 0, 1);  // 20260319 ZJH 先转置
...
TEST(TensorOpsTest, Slice) {
    auto t = om::Tensor::fromData(
        std::vector<float>{1, 2, 3, 4, 5, 6, 7, 8, 9}.data(), {3, 3});
    auto s = om::tensorSlice(t, 0, 1, 3);  // 20260319 ZJH 沿第 0 维截取 [1,3)，即第 1、2 行
...
TEST(TensorOpsTest, SliceDim1) {
    ...
    auto s = om::tensorSlice(t, 1, 0, 2);  // 20260319 ZJH 沿第 1 维截取 [0,2)，即前 2 列
...
TEST(TensorOpsTest, ScalarOps) {
    auto a = om::Tensor::fromData(std::vector<float>{1, 2, 3}.data(), {3});

    // 20260319 ZJH 加标量：[1,2,3] + 10 = [11,12,13]
    auto b = om::tensorAddScalar(a, 10.0f);
...
TEST(TensorOpsTest, Reductions) {
    auto t = om::Tensor::fromData(std::vector<float>{1, 2, 3, 4, 5}.data(), {5});
    EXPECT_FLOAT_EQ(om::tensorSum(t).item(), 15.0f);  // 20260319 ZJH 1+2+3+4+5=15，tensorSum 返回标量张量
    EXPECT_FLOAT_EQ(om::tensorMax(t), 5.0f);   // 20260319 ZJH 最大值为 5
    EXPECT_FLOAT_EQ(om::tensorMin(t), 1.0f);   // 20260319 ZJH 最小值为 1
}
