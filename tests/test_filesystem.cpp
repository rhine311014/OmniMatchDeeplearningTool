...
TEST(FileSystemTest, EnsureDir) {
    ...
    auto result = df::FileSystem::ensureDir(strNestedDir);
    ...
    auto result2 = df::FileSystem::ensureDir(strNestedDir);
...
TEST(FileSystemTest, ReadWriteText) {
    ...
    df::FileSystem::ensureDir(s_strTestRoot);

    std::string strFilePath = s_strTestRoot + "/hello.txt";  // 临时文本文件路径
    // 20260319 ZJH 包含换行和特殊字符，测试文本内容的完整性保留
    std::string strContent = "Hello, DeepForge!\n第二行内容\n特殊字符: <>&\"";

    // 20260319 ZJH 写入文本文件
    auto writeResult = df::FileSystem::writeText(strFilePath, strContent);
    ASSERT_TRUE(writeResult.has_value()) << "writeText 应成功写入文件";

    // 20260319 ZJH 读回文本内容，并与原始字符串逐字节比较
    auto readResult = df::FileSystem::readText(strFilePath);
...
TEST(FileSystemTest, ReadWriteBinary) {
    ...
    auto writeResult = df::FileSystem::writeBinary(strFilePath, vecOriginal);
    ASSERT_TRUE(writeResult.has_value()) << "writeBinary 应成功写入二进制文件";

    // 20260319 ZJH 读回二进制数据
    auto readResult = df::FileSystem::readBinary(strFilePath);
...
TEST(FileSystemTest, Exists) {
    ...
    EXPECT_FALSE(df::FileSystem::exists("nonexistent_path_xyz_12345"))
        << "不存在的路径应返回 false";

    // 20260319 ZJH 创建一个真实文件，验证 exists() 返回 true
    df::FileSystem::ensureDir(s_strTestRoot);
    std::string strFilePath = s_strTestRoot + "/exist_check.txt";
    df::FileSystem::writeText(strFilePath, "exists test");

    // 20260319 ZJH 已存在的文件路径应返回 true
    EXPECT_TRUE(df::FileSystem::exists(strFilePath))
        << "已创建的文件路径应返回 true";

    // 20260319 ZJH 已存在的目录路径也应返回 true
    EXPECT_TRUE(df::FileSystem::exists(s_strTestRoot))
        << "已创建的目录路径应返回 true";
...
TEST(FileSystemTest, ListFiles) {
    ...
    df::FileSystem::ensureDir(strListDir);

    // 20260319 ZJH 创建 3 个 .txt 文件和 2 个 .bin 文件，用于验证扩展名过滤
    df::FileSystem::writeText(strListDir + "/file1.txt", "text file 1");
    df::FileSystem::writeText(strListDir + "/file2.txt", "text file 2");
    df::FileSystem::writeText(strListDir + "/file3.txt", "text file 3");
    df::FileSystem::writeBinary(strListDir + "/data1.bin", {0x01, 0x02});
    df::FileSystem::writeBinary(strListDir + "/data2.bin", {0x03, 0x04});

    // 20260319 ZJH 按 .txt 扩展名过滤，应只返回 3 个文件
    auto resultTxt = df::FileSystem::listFiles(strListDir, ".txt");
    ...
    auto resultAll = df::FileSystem::listFiles(strListDir, "");
    ...
    auto resultNone = df::FileSystem::listFiles(strListDir, ".xyz");
...
TEST(FileSystemTest, ReadNonExistent) {
    ...
    auto textResult = df::FileSystem::readText("definitely_nonexistent_file_abc.txt");
    // 20260319 ZJH 期望返回失败（has_value() == false）
    EXPECT_FALSE(textResult.has_value()) << "读取不存在的文件应返回错误";
    // 20260319 ZJH 错误码应为 FileNotFound，确保语义正确
    EXPECT_EQ(textResult.error().code, df::ErrorCode::FileNotFound)
        << "错误码应为 FileNotFound";

    // 20260319 ZJH 同样验证 readBinary 对不存在文件的错误处理
    auto binResult = df::FileSystem::readBinary("definitely_nonexistent_file_abc.bin");
    EXPECT_FALSE(binResult.has_value()) << "读取不存在的二进制文件应返回错误";
    EXPECT_EQ(binResult.error().code, df::ErrorCode::FileNotFound)
