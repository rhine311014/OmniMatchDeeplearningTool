...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_bn1.buffers());  // 20260328 ZJH bn1 running_mean/running_var
        append(m_bn2.buffers());  // 20260328 ZJH bn2 running_mean/running_var
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("bn1", m_bn1);  // 20260328 ZJH bn1 缓冲区
        appendBufs("bn2", m_bn2);  // 20260328 ZJH bn2 缓冲区
        return vecResult;
    }
...
public:
    ...
    Tensor forward(const Tensor& input) override {
        ...
        auto output = tensorReshape(cx, {nBatch, nTotalPreds, nPredPerAnchor});
        return output;
    }
    ...
    std::vector<Tensor*> buffers() override {
        std::vector<Tensor*> vecResult;
        auto vecBn1 = m_bn1.buffers();  // 20260328 ZJH bn1 running_mean/running_var
        vecResult.insert(vecResult.end(), vecBn1.begin(), vecBn1.end());
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        std::vector<std::pair<std::string, Tensor*>> vecResult;
        std::string strBn1Prefix = strPrefix.empty() ? "bn1" : strPrefix + ".bn1";
        auto vecBufs = m_bn1.namedBuffers(strBn1Prefix);  // 20260328 ZJH bn1 缓冲区
        vecResult.insert(vecResult.end(), vecBufs.begin(), vecBufs.end());
        return vecResult;
    }
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_stemBn.buffers());  // 20260328 ZJH stem BN 缓冲区
        append(m_bn1.buffers());     // 20260328 ZJH bn1 缓冲区
        append(m_csp1.buffers());    // 20260328 ZJH csp1 内部 BN 缓冲区
        append(m_bn2.buffers());     // 20260328 ZJH bn2 缓冲区
        append(m_csp2.buffers());    // 20260328 ZJH csp2 内部 BN 缓冲区
        append(m_bn3.buffers());     // 20260328 ZJH bn3 缓冲区
        append(m_csp3.buffers());    // 20260328 ZJH csp3 内部 BN 缓冲区
        append(m_bn4.buffers());     // 20260328 ZJH bn4 缓冲区
        append(m_csp4.buffers());    // 20260328 ZJH csp4 内部 BN 缓冲区
        append(m_head.buffers());    // 20260328 ZJH head 内部 BN 缓冲区
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("stemBn", m_stemBn);
        appendBufs("bn1", m_bn1);   appendBufs("csp1", m_csp1);
        appendBufs("bn2", m_bn2);   appendBufs("csp2", m_csp2);
        appendBufs("bn3", m_bn3);   appendBufs("csp3", m_csp3);
        appendBufs("bn4", m_bn4);   appendBufs("csp4", m_csp4);
        appendBufs("head", m_head);
        return vecResult;
    }
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_bnReduce.buffers());  // 20260328 ZJH bnReduce running_mean/running_var
        append(m_bnBot1.buffers());    // 20260328 ZJH bnBot1 running_mean/running_var
        append(m_bnBot2.buffers());    // 20260328 ZJH bnBot2 running_mean/running_var
        append(m_bnExpand.buffers());  // 20260328 ZJH bnExpand running_mean/running_var
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("bnReduce", m_bnReduce);
        appendBufs("bnBot1", m_bnBot1);
        appendBufs("bnBot2", m_bnBot2);
        appendBufs("bnExpand", m_bnExpand);
        return vecResult;
    }
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_clsBn1.buffers());  // 20260328 ZJH 分类 bn1 缓冲区
        append(m_clsBn2.buffers());  // 20260328 ZJH 分类 bn2 缓冲区
        append(m_regBn1.buffers());  // 20260328 ZJH 回归 bn1 缓冲区
        append(m_regBn2.buffers());  // 20260328 ZJH 回归 bn2 缓冲区
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("clsBn1", m_clsBn1);  appendBufs("clsBn2", m_clsBn2);
        appendBufs("regBn1", m_regBn1);  appendBufs("regBn2", m_regBn2);
        return vecResult;
    }
...
private:
    int m_nClasses;              // 20260320 ZJH 类别数
    // 20260320 ZJH 分类分支
    Conv2d m_clsConv1, m_clsConv2;     // 20260320 ZJH ���类 3x3 卷积
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_stemBn.buffers());   // 20260328 ZJH stem BN 缓冲区
        append(m_bn1.buffers());      append(m_c2f1.buffers());  // 20260328 ZJH stage1 BN 缓冲区
        append(m_bn2.buffers());      append(m_c2f2.buffers());  // 20260328 ZJH stage2 BN 缓冲区
        append(m_bn3.buffers());      append(m_c2f3.buffers());  // 20260328 ZJH stage3 BN 缓冲区
        append(m_bn4.buffers());      append(m_c2f4.buffers());  // 20260328 ZJH stage4 BN 缓冲区
        append(m_head.buffers());     // 20260328 ZJH head 内部 BN 缓冲区
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("stemBn", m_stemBn);
        appendBufs("bn1", m_bn1);  appendBufs("c2f1", m_c2f1);
        appendBufs("bn2", m_bn2);  appendBufs("c2f2", m_c2f2);
        appendBufs("bn3", m_bn3);  appendBufs("c2f3", m_c2f3);
        appendBufs("bn4", m_bn4);  appendBufs("c2f4", m_c2f4);
        appendBufs("head", m_head);
        return vecResult;
    }
...
private:
    ...
    Conv2d m_down1, m_down2, m_down3, m_down4;   // 20260320 ZJH 下采样卷积
    BatchNorm2d m_bn1, m_bn2, m_bn3, m_bn4;      // 20260320 ZJH BN
    C2fBlock m_c2f1, m_c2f2, m_c2f3, m_c2f4;     // 20260320 ZJH C2f ���
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_bn1.buffers());    // 20260328 ZJH bn1 running_mean/running_var
        append(m_bn2.buffers());    // 20260328 ZJH bn2 running_mean/running_var
        append(m_bn3.buffers());    // 20260328 ZJH bn3 running_mean/running_var
        append(m_bn4.buffers());    // 20260328 ZJH bn4 running_mean/running_var
        append(m_bnOut.buffers());  // 20260328 ZJH bnOut running_mean/running_var
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("bn1", m_bn1);    appendBufs("bn2", m_bn2);
        appendBufs("bn3", m_bn3);    appendBufs("bn4", m_bn4);
        appendBufs("bnOut", m_bnOut);
        return vecResult;
    }
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_stemBn.buffers());   // 20260328 ZJH stem BN 缓冲区
        append(m_dbn1.buffers());     append(m_elan1.buffers());  // 20260328 ZJH stage1 BN 缓冲区
        append(m_dbn2.buffers());     append(m_elan2.buffers());  // 20260328 ZJH stage2 BN 缓冲区
        append(m_dbn3.buffers());     append(m_elan3.buffers());  // 20260328 ZJH stage3 BN 缓冲区
        append(m_head.buffers());     // 20260328 ZJH head 内部 BN 缓冲区
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("stemBn", m_stemBn);
        appendBufs("dbn1", m_dbn1);  appendBufs("elan1", m_elan1);
        appendBufs("dbn2", m_dbn2);  appendBufs("elan2", m_elan2);
        appendBufs("dbn3", m_dbn3);  appendBufs("elan3", m_elan3);
        appendBufs("head", m_head);
        return vecResult;
    }
...
public:
    ...
    std::vector<Tensor*> buffers() override {
        ...
        append(m_stemBn.buffers());   // 20260328 ZJH stem BN 缓冲区
        append(m_down1.buffers());    append(m_c2f1.buffers());  // 20260328 ZJH stage1 BN 缓冲区
        append(m_down2.buffers());    append(m_c2f2.buffers());  // 20260328 ZJH stage2 BN 缓冲区
        append(m_down3.buffers());    append(m_c2f3.buffers());  // 20260328 ZJH stage3 BN 缓冲区
        append(m_head.buffers());     // 20260328 ZJH head 内部 BN 缓冲区
        return vecResult;
    }
    ...
    std::vector<std::pair<std::string, Tensor*>> namedBuffers(const std::string& strPrefix = "") override {
        ...
        appendBufs("stemBn", m_stemBn);
        appendBufs("down1", m_down1);  appendBufs("c2f1", m_c2f1);
        appendBufs("down2", m_down2);  appendBufs("c2f2", m_c2f2);
        appendBufs("down3", m_down3);  appendBufs("c2f3", m_c2f3);
        appendBufs("head", m_head);
        return vecResult;
    }
