// 20260319 ZJH DeepForge Phase 2 — MNIST 训练主程序（MLP / ResNet18 双模式）
// 命令行参数:
...
int main(int argc, char* argv[]) {
    ...
    std::printf("========================================\n");
    std::printf("  DeepForge Phase 2 - MNIST Train\n");
    if (bUseResNet) {
    ...
    std::shared_ptr<df::Module> pModel;  // 20260319 ZJH 通用模型指针
    ...
    if (bUseResNet) {
        ...
        pMLP->add(std::make_shared<df::Linear>(nInputDim, nHiddenDim));   // 20260319 ZJH 第一层：784->128
        pMLP->add(std::make_shared<df::ReLU>());                           // 20260319 ZJH ReLU 激活
        pMLP->add(std::make_shared<df::Linear>(nHiddenDim, nOutputDim));  // 20260319 ZJH 第二层：128->10
        pModel = pMLP;
    }
    ...
    std::unique_ptr<df::Adam> pAdamOpt;    // 20260319 ZJH Adam 优化器（ResNet 使用）
    std::unique_ptr<df::SGD> pSgdOpt;      // 20260319 ZJH SGD 优化器（MLP 使用）

    if (bUseResNet) {
        pAdamOpt = std::make_unique<df::Adam>(vecParams, fLearningRate);  // 20260319 ZJH 创建 Adam
    } else {
        pSgdOpt = std::make_unique<df::SGD>(vecParams, fLearningRate);    // 20260319 ZJH 创建 SGD
    }
